...
/* RT code for breaking link */
	...
	if (!str_cmp(arg,"help"))
	{
	    ...
	    if (argument[0] == '\0')
	      {
		sprintf(buf,
"The MUD is home to %d different races with brief descriptions below:",
			MAX_PC_RACE - 1);
...
//        ORG_RACE(ch) = race;
	...
	ch->size = pc_race_table[race].size;
        ...
        case CON_GET_NEW_CLASS:
            ...
            write_to_buffer (d, "\n\r", 0);
            send_to_desc ("who is your god (help for more information)? ",
                          d);
...
//            argument = one_argument(argument,arg);
		...
		if(mud_telnetga)
            ...
            sprintf(buf, "{r%s {whas entered {YCotN 5.0 {bQuickMud Edition.{x", ch->name);
            enter_info(buf);
