...
void boot_db ()
{
	...
	if(!mysql_real_connect(mydb, "localhost", "username", "password", "database", 0, NULL, 0 ) )
...
CHAR_DATA *create_mobile (MOB_INDEX_DATA * pMobIndex)
{
	...
	mob->max_hit += mob->max_hit * 4;
        ...
        if (mob->level > 10)
        mob->damage[DICE_NUMBER] += mob->damage[DICE_NUMBER] * 4;
        if (mob->level > 10)
	mob->damage[DICE_TYPE] += mob->damage[DICE_TYPE] * 4;
        ...
        if (IS_SET (mob->act, ACT_NECROMANCER))
        {
            mob->perm_stat[STAT_INT] += 3;
            mob->perm_stat[STAT_STR] -= 1;
            mob->perm_stat[STAT_DEX] += 1;
...
OBJ_DATA *create_object (OBJ_INDEX_DATA * pObjIndex, int level)
{
    ...
    switch (obj->item_type)
    {
        ...
        case ITEM_ROOM_KEY:
        case ITEM_GEM:
        case ITEM_JEWELRY:        case ITEM_SMITH_FURNACE:
        case ITEM_SMITH_ORE:
