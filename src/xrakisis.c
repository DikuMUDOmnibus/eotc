...
//        if (ch->level == 105) exptolevel == 500000;
//        if (ch->level == 104) exptolevel == 400000;
//        if (ch->level == 103) exptolevel == 300000;
//        if (ch->level == 102) exptolevel == 200000;
//        if (ch->level == 101) exptolevel == 100000;
//        if (ch->level == 100) exptolevel == 50000;
//        if (ch->level < 100) exptolevel == 4000;
...
//	ch->exp -= exptolevel; // exptolevel; 
	...
	sprintf(buf, "%s has gained a level.", ch->name);
	do_info(ch,buf);
	...
	return;
        ...
        if (ch->level2 >= 200)
        {
        send_to_char ("Your second class may only be level 200\n\r  ", ch);
        return;
        }
    ...
    add_mana = number_range (2, (2 * get_curr_stat (ch, STAT_INT)
                                 + get_curr_stat (ch, STAT_WIS)) / 5);
    if (!class_table[ch->class2].fMana)
        add_mana /= 2;
    add_move = number_range (1, (get_curr_stat (ch, STAT_CON)
                                 + get_curr_stat (ch, STAT_DEX)) / 6);
    add_prac += get_curr_stat(ch, STAT_WIS) / 10;
...
//        add_prac += 10;
	...
	return;
        ...
        if (ch->level3 >= 200)
        {
        send_to_char ("Your third class may only be level 200\n\r  ", ch);
        return;
        }
    ...
    add_mana = number_range (2, (2 * get_curr_stat (ch, STAT_INT)
                                 + get_curr_stat (ch, STAT_WIS)) / 5);
    if (!class_table[ch->class3].fMana)
        add_mana /= 2;
    add_move = number_range (1, (get_curr_stat (ch, STAT_CON)
                                 + get_curr_stat (ch, STAT_DEX)) / 6);
    add_prac += get_curr_stat(ch, STAT_DIS) / 10;
...
//      add_hp = add_hp * 30;
	...
	send_to_char (" Level with argument classone, classtwo or classthree to advance classes\n\r  ", ch);
...
void do_multiclass(CHAR_DATA *ch, char *argument)
{
  ...
  if (!IS_NPC(ch) && ch->class != -1 
      && ch->class2 != -1 && ch->class3 != -1 )
  {   
    send_to_char("You already have 3 classes.\n\r",ch);
    return;
  }
...
void do_circle( CHAR_DATA *ch, char *argument )
{
    ...
    && ch->level < skill_table[gsn_circle].skill_level[ch->class2]
    && ch->level < skill_table[gsn_circle].skill_level[ch->class3] )
...
*/
    ...
    {
        one_hit(ch, victim, gsn_dual_circle,FALSE);
        one_hit(ch, victim, gsn_dual_circle,FALSE);
        one_hit(ch, victim, gsn_dual_circle,FALSE);
...
//	int value;
	...
	if (!IS_OBJ_STAT (obj, ITEM_RELIC))
        {
	send_to_char("You may only Consecrate Relics.\n\r",ch);
	return;
	}
