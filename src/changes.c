...
void do_changes(CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf, "{y--{Y::::{y---{Y::{y-------------------------------------------------{Y::{y---{Y::::{y--{x\n\r"); stc(buf,ch);
  sprintf(buf, "          {mCode Changes to {YEye of the Cyclops{x               \n\r"); stc(buf,ch);
  sprintf(buf, "{y--{Y::::{y---{Y::{y-------------------------------------------------{Y::{y---{Y::::{y--{x\n\r\n\r"); stc(buf,ch);
  if ( is_number(arg))
  for ( i = (v-1)*10; i < (v*10); i++ )
  {
    if ( i >= max_change ) break;
    sprintf(buf, " {b[{w%3d{b] {g%-6s  {b%-12s {c%s{x\n\r",
  ...
  stc("\n\r",ch);
  sprintf(buf, "{y--{Y::::{y---{Y::{y-------------------------------------------------{Y::{y---{Y::::{y--{x\n\r"); stc(buf,ch);
  sprintf(buf, "      Type 'changes <1-%d>' to view a specific page of changes.\n\r",(max_change/10)+1); stc(buf,ch);
  sprintf(buf, "           There are a total of %d changes in the database.\n\r", max_change); stc(buf,ch);
  sprintf(buf, "{y--{Y::::{y---{Y::{y-------------------------------------------------{Y::{y---{Y::::{y--{x\n\r"); stc(buf,ch);
