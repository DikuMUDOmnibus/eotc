...
void do_score (CHAR_DATA * ch, char *argument)
{
    ...
    if (ch->level < 200){
    sprintf(buf, "{cExp to Level: {C %d/%d/%d{x\n\r",
			16000 - ch->exp, 124000 - ch->exp, 248000 - ch->exp);
    send_to_char(buf, ch);}
    ...
    if (HAS_HOME(ch))
	send_to_char("You currently own a home.\n\r", ch);
    else
...
void do_whois (CHAR_DATA * ch, char *argument)
{
	...
	if (ch->disease == -1) disease = "Normal  ";
	if (ch->disease == 0) disease = "Werewolf";
	if (ch->disease == 1) disease = "Vampire ";
...
void do_who (CHAR_DATA * ch, char *argument)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
       ...
       if (wch->disease == -1) disease = "Normal  ";
        if (wch->disease == 1) disease = "Werewolf";
        if (wch->disease == 0) disease = "Vampire ";
...
/*
         * Format it up.
         */
        sprintf (buf, "{b[{w%3d %3d %3d {b%6s {y%s{b] {R[{W%s{R]{x %s{x %s %s%s%s%s%s%s%s\n\r",
                 wch->level, wch->level2, wch->level3,
...
//                 clan_table[wch->clan].who_name,
    ...
    sprintf (buf2, "\n\rPlayers found: {B[{W%d{B]{x Most on Ever {B[{W%d{B]{x {WMudinfo{x for more stats\n\r", nMatch, max_ever);
    add_buf (output, buf2);
    page_to_char (buf_string (output), ch);
    free_buf (output);
    ...
    if (!global_exp)
    {
    sprintf(buf, "Double Exp is {B[{Roff{B]{x  ");
    send_to_char(buf, ch);
    }
    if (global_exp)
    {
    sprintf(buf, "Double Exp is {B[{Won{B]{x  ");
    send_to_char(buf, ch);
    }
    if (!double_stance)
    {
    sprintf(buf, "Double Stance is {B[{Roff{B]{x  ");
    send_to_char(buf, ch);
    }

    if (double_stance)
    {
    sprintf(buf, "Double Stance is {B[{Won{B]{x  ");
    send_to_char(buf, ch);
    }
    if (!double_improve)
    {
    sprintf(buf, "Double Improve is {B[{Roff{B]{x\n\r");
    send_to_char(buf, ch);
    }
    if (double_improve)
    {
    sprintf(buf, "Double Improve is {B[{Won{B]{x\n\r");
    send_to_char(buf, ch);
    }
    sprintf(buf, " {r({w*{r){g-------{r({w**{r){w= {yEye of the Cyclops {w={r({w**{r){g-------{r({w*{r){x\n\r");
    send_to_char(buf, ch);
